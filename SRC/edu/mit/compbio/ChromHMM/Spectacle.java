/**
 * Spectacle - Spectral learning for Annotating Chromatin Labels and Epigenomics (modified from ChromHMM)
 * Copyright (C) 2013-r014 Jimin Song
 * 
...
import Jama.*;	// Added by JS



/**
 * The main class of Spectacle implements command line parsing and core algorithms
 * The Spectacle code was written by Jason Ernst 
 */
public class Spectacle
{
    ...
    // Added by JS //
    /**
     * Constant for parameter initialization to be based on loading a model file with combinations of marks
     */
    static int INITMETHOD_LOAD_COMB = 3;
    // Added by JS //
    ...
    int nmaxprocessors;
		...
		// Added by JS // 
		/**
		 * Spectral learning is used for parameter estimation
		 */
		static int EXPECTATION_MAXIMIZATION = 0;    // Added by JS
		static int SPECTRAL_DETERMINISTIC  = 1;   // Added by JS
		static int SPECTRAL_RANDOM  = 2;        // Added by JS
		int ParameterSpectralLearning = SPECTRAL_DETERMINISTIC; // Added by JS
    boolean isComb = false; // Added by JS; observations are combinations of datasets (marks) without assumption of independence between datasets (marks).
		boolean computesamplemomentonly = false;
    double[][] emissionproductsSpectral; // Added by JS
		/**
		 * Stores the data smoothing factor lambda for spectral learning
		 */
		double lambda = 1.0; // Added by JS
		// Added by JS // 
     ...
     */
     public Spectacle(String szinputdir, String szoutputdir, String szinputfilelist,String szchromlengthfile, int numstates,int nseed, int ninitmethod,
...
//		     Color theColor, boolean bnormalEM, int nmaxprocessors, int ParameterSpectralLearning, double lambda, boolean isComb) throws IOException	// Added by JS
		     Color theColor, boolean bnormalEM, int nmaxprocessors, int ParameterSpectralLearning, double lambda, boolean isComb, boolean computesamplemomentonly) throws IOException	// Added by JS
    {
	...
	this.chorder = Spectacle.ORDERCHARS[nstateorder];
	this.szorder = Spectacle.ORDERSTRINGS[nstateorder];
	...
	this.ParameterSpectralLearning = ParameterSpectralLearning; // Added by JS
	this.lambda = lambda; // Added by JS
  this.isComb = isComb; // Added by JS
  this.computesamplemomentonly = computesamplemomentonly; // Added by JS
    ...
    ///////////////////////////////////////////////////////////////////////////////////////////////////////////////
      ...
      */
    public Spectacle(String szInitFile, String szoutputdir, String szstateorderingfile, String szcolumnorderingfile,
     ...
     */
    public Spectacle(String szinputdir, String szinputfilelist, String szchromlengthfile, String szoutputdir, String szInitFile, String szoutfileID,
//                    int nbinsize, boolean bprintposterior, boolean bprintsegment,boolean bprintstatebyline) throws IOException
                    int nbinsize, boolean bprintposterior, boolean bprintsegment,boolean bprintstatebyline,boolean isComb) throws IOException	// Added by JS
    {
	this.szinputdir = szinputdir;
	this.szinputfilelist = szinputfilelist;
	this.szchromlengthfile = szchromlengthfile;
...
//	loadModel();
     ...
     */
    public Spectacle(String szinputdir, String szsegmentdir, String szinputfilelist, String szconfusionfileprefix,
    ...
    {
	for (int ni = 1; ni < Spectacle.ORDERCHARS.length; ni++)
	{
	    if (Spectacle.ORDERCHARS[ni] == ch)
	    {
		return Spectacle.ORDERSTRINGS[ni];
	    }
	}
	return Spectacle.ORDERSTRINGS[0];
    }
    ...
    public void buildModel() throws IOException
    {
			...
			if (ParameterSpectralLearning!=EXPECTATION_MAXIMIZATION) {
				trainParametersSpectral(ParameterSpectralLearning,isComb,computesamplemomentonly);
				if (!computesamplemomentonly) {
                if (bnormalEM) {
                    trainParametersParallel(isComb);
                } else {
                    trainParameters(isComb);
                }
				}
			} else {
			// Added by JS //
       if (ninitmethod ==Spectacle.INITMETHOD_LOAD) 
       {
	   //loads parameters of the initial model
	   loadModelSmooth(dloadsmoothemission,dloadsmoothtransition,false);  // Modified by JS
       }
       // Added by JS //
       else if (ninitmethod ==Spectacle.INITMETHOD_LOAD_COMB)
       {
         //loads parameters of the initial model
         loadModelSmooth(dloadsmoothemission,dloadsmoothtransition,true);
       }
       // Added by JS //
       else if (ninitmethod == Spectacle.INITMETHOD_INFORMATION)
       {
	   informationInitializeNested();
       }
       else if (ninitmethod == Spectacle.INITMETHOD_RANDOM)
       {
...
//          trainParametersParallel();
           trainParametersParallel(isComb); // Added by JS
       }
       else
       {
//	   trainParameters();
           trainParameters(isComb); // Added by JS
       }
			// Added by JS //
			}	
...
//			System.out.println("runtime: "+(time-start));
    ...
    ///////////////////////////////////////////////////////////////////////////////////////////////////////////////
    /**
     * If nstateorder is Spectacle.STATEORDER_EMISSION orders emission parameters based on emission parameters and stores in stateordering
     * If nstateorder is Spectacle.STATEORDER_TRANSITION orders emission parameters based on transition parameters and stores in stateordering
     */
    private void makeStateOrdering()
    {
	if (nstateorder ==  Spectacle.STATEORDER_EMISSION)
	{
	...
	else if (nstateorder == Spectacle.STATEORDER_TRANSITION)
	{
    ...
    {
//	loadModelSmooth(0,0);
        loadModelSmooth(0,0,false); // Modified by JS
    }
...
//    public void loadModelSmooth(double dproceduresmoothemission, double dproceduresmoothtransition) throws IOException
    public void loadModelSmooth(double dproceduresmoothemission, double dproceduresmoothtransition, boolean isComb) throws IOException
    {
	...
	if ((nstateorder != Spectacle.STATEORDER_TRANSITION)&&(nstateorder != Spectacle.STATEORDER_EMISSION))
	{
	   nstateorder = -1;
	   for (int ni = 0; ni < Spectacle.ORDERCHARS.length; ni++)
	   {
	      if (chorder == Spectacle.ORDERCHARS[ni])
	      {
	         nstateorder = ni;
	         break;
	      }
	   ...
	   szorder = Spectacle.ORDERSTRINGS[nstateorder];
	}
	...
	//initialize the transition structure without any parameter elimination
	   ...
	   transitionprobsnumCol[ni] = numstates;
        ...
        emissionprobs = new double[numstates][numdatasets][numbuckets];
        // Added by JS //
        int nobserved0 = (int) Math.pow(2, numdatasets);
        int[] transObs = new int[nobserved0];;
        if (isComb) {
            for (int i = 0; i < nobserved0; i++) {
                transObs[i] = -1;
            }
            for (int ni = 0; ni < traindataObservedValues.length; ni++) {
                int idx = 0;
                for (int nd = 0; nd < numdatasets; nd++) {
                    idx *= 2;
                    if (traindataObservedValues[ni][nd] == true) {
                        idx += 1;
                    }
                }
                transObs[idx] = ni;
            }
            emissionproductsSpectral = new double[traindataObservedValues.length][numstates];
            for (int nj = 0; nj < traindataObservedValues.length; nj++) {
                emissionproductsSpectral[nj] = new double[numstates];
								for (int ns = 0; ns < numstates; ns++) {
									emissionproductsSpectral[nj][ns] = 1e-20;
								}
            }
            for (int ns=0; ns<numstates; ns++) {
                for (int nd=0; nd<numdatasets; nd++) {
                    emissionprobs[ns][nd][1] = 0.0;
                }
            }
        }
        // Added by JS //
	...
	while ((szLine = br.readLine())!=null)
	{
	    ...
	    else if (szvartype.equalsIgnoreCase("emissionprobs")&(isComb==false)) // Modified by JS
	    {
		int nstate = Integer.parseInt(st.nextToken())-1;
        ...
        // Added by JS //
        else if (szvartype.equalsIgnoreCase("emissionprobs")&(isComb==true)) // Modified by JS
        {
            int nstate = Integer.parseInt(st.nextToken())-1;
            int obs_comb = Integer.parseInt(st.nextToken());
            double dprob = Double.parseDouble(st.nextToken());
            if (transObs[obs_comb]!=-1) {
                emissionproductsSpectral[transObs[obs_comb]][nstate] = Math.max(1e-20,dprob);
            }
            String bin = Integer.toBinaryString(obs_comb);
            for (int nd=0; nd<bin.length(); nd++) {
                if (bin.charAt(bin.length()-nd-1)=='1') {
                    emissionprobs[nstate][numdatasets-nd-1][1] += dprob;
                }
            }
        }
        // Added by JS //
	...
	br.close();
        ...
        for (int ns=0; ns<numstates; ns++) {
            for (int nd=0; nd<numdatasets; nd++) {
                emissionprobs[ns][nd][1] = Math.max(1e-20,emissionprobs[ns][nd][1]);
                emissionprobs[ns][nd][0] = Math.max(1e-20,1 -  emissionprobs[ns][nd][1]);
            }
        }
        // Added by JS //
    ...
    public void makeSegmentationConfusion() throws IOException
    {
       ...
       int nsparsecutoff = (int) (numstates * Spectacle.SPARSECUTOFFRATIO);
	  ...
	  if (breadposterior)
	  {
	     ...
	     String szposteriorinfilename = szsegmentdir+"/POSTERIOR/"+szprefix+"_"+chromSeq[nordered_nseq]+Spectacle.SZPOSTERIOREXTENSION;
          ...
          else if (breadstatebyline)
	  {
	      ...
	      String szmaxinfilename = szsegmentdir+"/STATEBYLINE/"+szprefix+"_"+szcurrchrom+Spectacle.SZSTATEBYLINEEXTENSION;
	  ...
	  else if (breadsegment)
	  {
	     ...
	     String szsegmentinfilename = szsegmentdir+"/" + szprefix+Spectacle.SZSEGMENTEXTENSION;
...
//    public void makeSegmentation() throws IOException
    public void makeSegmentation(boolean isComb) throws IOException // Added by JS
    {
        NumberFormat nf = NumberFormat.getInstance();
        nf.setMaximumFractionDigits(4);
        // Added by JS //
          ...
          if (numtime[nseq] > nmaxtime)
	  {
       ...
       //stores the emission probability for the i^th combination of marks in the j^th state
//       double[][] emissionproducts = new double[traindataObservedValues.length][numstates];
       double[][] emissionproducts; // Added by JS
       ...
       hsprefix = new HashSet();
			 ...
			 double loglike = 0.0; // Added by JS
       for (int nseq = 0; nseq < traindataObservedIndex.length; nseq++)
       {
	   int nordered_nseq = ordered[nseq].nindex;
	   //goes through each sequence
	  ...
	  if (bprintposterior)
	  {
	      ...
	      String szposterioroutfilename = szoutputdir+"/POSTERIOR/"+szprefix+"_"+chromSeq[nordered_nseq]+Spectacle.SZPOSTERIOREXTENSION;
	  ...
	  if (bprintstatebyline)
	  {
	      ...
	      String szmaxoutfilename = szoutputdir+"/STATEBYLINE/"+szprefix+"_"+chromSeq[nordered_nseq]+Spectacle.SZSTATEBYLINEEXTENSION;
      ...
      if (isComb) {
          emissionproducts = emissionproductsSpectral;
      } else {
          emissionproducts = new double[traindataObservedValues.length][numstates];
          for (int ni = 0; ni < emissionproducts.length; ni++) {
              //going through each combination of marks
              if (traindataObservedSeqFlags_nseq[ni]) {
                  //this signature of marks is observed on the current chromosome so
...
//		loglike += Math.log(dmaxval);	    // Added by JS
      loglike += Math.log(scale[0]);	    // Added by JS
	  ...
	  int nmaxstateprev = nmaxstate;  	
		...
		double dprob = gamma_nt[nmappedstate];
 	        if (bprintposterior)
	        {
	           ...
	           {
                      pwprobs.print(nf.format(dprob));              // Modified by JS
		   }
		}
...
//			 loglike += Math.log(dmaxval);	    // Added by JS
              loglike += Math.log(scale[nt]);	    // Added by JS
	  ...
	  if (bprintposterior)
	  {
       ...
       if (bprintsegment)
       {
	     ...
	     pw.close();
			 ...
			 System.out.println("Estimated Log Likelihood: "+nf3.format(loglike));	// Added by JS
...
//    public void trainParameters() throws IOException
    public void trainParameters(boolean isComb) throws IOException // Added by JS
    {
        NumberFormat nf3 = NumberFormat.getInstance();
        nf3.setMaximumFractionDigits(3);
	nf3.setGroupingUsed(false);
	...
	nf1.setGroupingUsed(false);
       ...
       int nsparsecutofflooser = (int) (numstates * Spectacle.SPARSECUTOFFLOOSERRATIO);
...
//       double[][] emissionproducts = new double[traindataObservedValues.length][numstates];
       ...
       do
       {
		...
		beta_ntp1 = beta_nt;  //updating beta_ntp1 
	     ...
	     //M step
         if ((nmaxiterations>0)&((niteration >1) ||(nseq==(traindataObservedIndex.length-1))))  // Added by JS
...
//    public void trainParametersParallel() throws IOException
    public void trainParametersParallel(boolean isComb) throws IOException // Added by JS
    {
        NumberFormat nf3 = NumberFormat.getInstance();
        nf3.setMaximumFractionDigits(3);
	nf3.setGroupingUsed(false);
...
//       double[][] emissionproducts = new double[traindataObservedValues.length][numstates];
    ...
    ////////////////////////////////////////////////////////////////////////////////////////////////////////
		...
		/**
		 * This is the core procedure for learning the parameters of the model using spectral learning
			 Added by JS
		 */
		public void trainParametersSpectral(int ParameterSpectralLearning, boolean isComb, boolean computesamplemomentonly) throws IOException
//        public void trainParametersSpectral() throws IOException
		{
            NumberFormat nf = NumberFormat.getInstance();
//            nf.setMaximumFractionDigits(4);
            nf = new DecimalFormat("0.####E0");

            // Read data- #segments, #non-zero observations
            int nobserved = traindataObservedValues.length;
            int nObservation = 0;
            int[] observed = new int[nobserved];
            for (int ni=0; ni<nobserved; ni++) {
                observed[ni] = 0;
            }
            for (int nfile = 0; nfile < chromfiles.length; nfile++) {
                int nsize = traindataObservedIndex[nfile].length;
                nObservation += nsize;
                for (int nrow = 0; nrow < nsize; nrow++) {
                    observed[traindataObservedIndex[nfile][nrow]] = 1;
                }
            }
            int[] transObs = new int[nobserved];
            int nobserved2 = 0;
            for (int ni=0; ni<nobserved; ni++) {
                if (observed[ni]==1) {
                    transObs[ni] = nobserved2++;
                } else {
                    transObs[ni] = -1;
                }
            }
            nobserved = nobserved2;
            int[] transObs2 = new int[nobserved];
            int nobserved0 = (int)Math.pow(2,numdatasets);
            int[] transObs0 = new int[nobserved0];
            int[] transObs02 = new int[nobserved];
            for (int i=0; i<nobserved0; i++) {
                transObs0[i] = -1;
            }
            for (int ni=0; ni<traindataObservedValues.length; ni++) {
                if (transObs[ni]!=-1) { transObs2[transObs[ni]] = ni;   }
                int idx = 0;
                for (int nd=0; nd<numdatasets; nd++) {
                    idx *= 2;
                    if (traindataObservedValues[ni][nd]==true) {   idx += 1;   }
                }
                transObs0[idx] = ni;
                transObs02[transObs[ni]] = idx;
            }
            System.out.println("Number of segments: "+nObservation);
            System.out.println("Number of non-zero combinations of marks: "+nobserved);

            // Read data- observation singletons, pairs, and triples
            Matrix observationSingleMatrix = new Matrix(nobserved,1,0);
            HashMap<String,Double> observationPair21 = new HashMap<String,Double>();
            HashMap<String,Double> observationPair31 = new HashMap<String,Double>();
            HashMap<String,Double> observationTriple = new HashMap<String,Double>();
            for (int nfile = 0; nfile < chromfiles.length; nfile++) {
                String nobservedCurr = "";
                String nobservedPrev = "";
                String nobservedPrevPrev = "";
                int nsize = traindataObservedIndex[nfile].length;
                for (int nrow = 0; nrow < nsize; nrow++) {
										int curr = transObs[traindataObservedIndex[nfile][nrow]];
                    nobservedCurr = String.valueOf(curr);
                    observationSingleMatrix.set(curr,0,observationSingleMatrix.get(curr,0)+1.0/(double)nObservation);
                    if (nrow>=1) {
                        Double d = observationPair21.get(nobservedPrev+","+nobservedCurr);
                        if (d==null) {  observationPair21.put(nobservedPrev+","+nobservedCurr,Double.valueOf(1.0/(double)(nObservation-1)));  }
                        else {  observationPair21.put(nobservedPrev+","+nobservedCurr,Double.valueOf(d.doubleValue()+1.0/(double)(nObservation-1)));  }
                    }
                    if (nrow>=2) {
                        Double d = observationPair31.get(nobservedPrevPrev+","+nobservedCurr);
                        if (d==null) {  observationPair31.put(nobservedPrevPrev+","+nobservedCurr,Double.valueOf(1.0/(double)(nObservation-2)));  }
                        else {  observationPair31.put(nobservedPrevPrev+","+nobservedCurr,Double.valueOf(d.doubleValue()+1.0/(double)(nObservation-2)));  }
                        d = observationTriple.get(nobservedPrevPrev+","+nobservedPrev+","+nobservedCurr);
                        if (d==null) {  observationTriple.put(nobservedPrevPrev+","+nobservedPrev+","+nobservedCurr,Double.valueOf(1.0/(double)(nObservation-2)));  }
                        else {  observationTriple.put(nobservedPrevPrev+","+nobservedPrev+","+nobservedCurr,Double.valueOf(d.doubleValue()+1.0/(double)(nObservation-2)));  }
                    }
                    nobservedPrevPrev = nobservedPrev;
                    nobservedPrev = nobservedCurr;
                }
            }
//            System.out.println("Number of pairs21: "+observationPair21.size());
//            System.out.println("Number of pairs31: "+observationPair31.size());
//            System.out.println("Number of triples: "+observationTriple.size());


            // Smooth out pairs and triples data
            // if lambda==1, this part will be skipped.
            HashMap<String, Double> observationPairPrime21;
            HashMap<String, Double> observationPairPrime31;
            HashMap<String, Double> observationTriplePrime;
            Iterator it;
            if (lambda<1) {
                HashMap<String, Double> observationPair1 = new HashMap<String, Double>();
                HashMap<String, Double> observationPair2 = new HashMap<String, Double>();
                it = observationPair21.entrySet().iterator();
                while (it.hasNext()) {
                    Map.Entry<String, Double> pair = (Map.Entry<String, Double>) it.next();
                    StringTokenizer st = new StringTokenizer(pair.getKey(), ",");
                    int k1 = Integer.valueOf(st.nextToken()).intValue();
                    int k2 = Integer.valueOf(st.nextToken()).intValue();
                    Double Val = observationPair1.remove(String.valueOf(k1));
                    if (Val != null) {
                        observationPair1.put(String.valueOf(k1), Val.doubleValue() + pair.getValue().doubleValue());
                    } else {
                        observationPair1.put(String.valueOf(k1), pair.getValue().doubleValue());
                    }
                    Val = observationPair2.remove(String.valueOf(k2));
                    if (Val != null) {
                        observationPair2.put(String.valueOf(k2), Val.doubleValue() + pair.getValue().doubleValue());
                    } else {
                        observationPair2.put(String.valueOf(k2), pair.getValue().doubleValue());
                    }
                }
                observationPairPrime21 = new HashMap<String, Double>();
                it = observationPair21.entrySet().iterator();
                double sum = 0;
                while (it.hasNext()) {
                    Map.Entry<String, Double> pair = (Map.Entry<String, Double>) it.next();
                    ...
                    int k1 = Integer.valueOf(st.nextToken()).intValue();
                    int k2 = Integer.valueOf(st.nextToken()).intValue();
                    Double Val1 = observationPair1.get(String.valueOf(k1));
                    Double Val2 = observationPair2.get(String.valueOf(k2));
                    if ((Val1 != null) & (Val2 != null)) {
                        double moment1 = pair.getValue().doubleValue();
                        double moment2 = Val1.doubleValue() * Val2.doubleValue();
                        double moment = lambda * moment1 + (1 - lambda) * moment2;
                        sum += moment;
                        observationPairPrime21.put(String.valueOf(k1) + "," + String.valueOf(k2), moment);
                    }
                }
                it = observationPairPrime21.entrySet().iterator();
                while (it.hasNext()) {
                    Map.Entry<String, Double> pair = (Map.Entry<String, Double>) it.next();
                    observationPairPrime21.put(pair.getKey(), pair.getValue().doubleValue() / sum);
                }
                observationPair21.clear();
                observationPair1.clear();
                observationPair2.clear();
                it = observationPair31.entrySet().iterator();
                while (it.hasNext()) {
                    Map.Entry<String, Double> pair = (Map.Entry<String, Double>) it.next();
                    StringTokenizer st = new StringTokenizer(pair.getKey(), ",");
                    int k1 = Integer.valueOf(st.nextToken()).intValue();
                ...
                observationPairPrime31 = new HashMap<String, Double>();
                it = observationPair31.entrySet().iterator();
                sum = 0;
                while (it.hasNext()) {
                    Map.Entry<String, Double> pair = (Map.Entry<String, Double>) it.next();
                    ...
                    if ((Val1 != null) & (Val2 != null)) {
                        ...
                        double moment = lambda * moment1 + (1 - lambda) * moment2;
                        sum += moment;
                        observationPairPrime31.put(String.valueOf(k1) + "," + String.valueOf(k2), moment);
                    }
                }
                it = observationPairPrime31.entrySet().iterator();
                while (it.hasNext()) {
                    Map.Entry<String, Double> pair = (Map.Entry<String, Double>) it.next();
                    observationPairPrime31.put(pair.getKey(), pair.getValue().doubleValue() / sum);
                }
                observationPair31.clear();
                observationPair1.clear();
                observationPair2.clear();
                HashMap<String, Double>[] observationTriple1 = new HashMap[3];
                HashMap<String, Double>[] observationTriple2 = new HashMap[3];
                for (int i = 0; i < 3; i++) {
                    observationTriple1[i] = new HashMap<String, Double>();
                    observationTriple2[i] = new HashMap<String, Double>();
                }
                it = observationTriple.entrySet().iterator();
                while (it.hasNext()) {
                    Map.Entry<String, Double> pair = (Map.Entry<String, Double>) it.next();
                    StringTokenizer st = new StringTokenizer(pair.getKey(), ",");
                    int[] idx = new int[3];
                    idx[0] = Integer.valueOf(st.nextToken()).intValue();
                    idx[1] = Integer.valueOf(st.nextToken()).intValue();
                    idx[2] = Integer.valueOf(st.nextToken()).intValue();
                    for (int i = 0; i < 3; i++) {
                        Double Val = observationTriple1[i].remove(String.valueOf(idx[i]));
                        if (Val != null) {
                            observationTriple1[i].put(String.valueOf(idx[i]), Val.doubleValue() + pair.getValue().doubleValue());
                        } else {
                            observationTriple1[i].put(String.valueOf(idx[i]), pair.getValue().doubleValue());
                        }
                        String key = "";
                        if (i == 0) {
                            key = String.valueOf(idx[1]) + "," + String.valueOf(idx[2]);
                        } else if (i == 1) {
                            key = String.valueOf(idx[0]) + "," + String.valueOf(idx[2]);
                        } else if (i == 2) {
                            key = String.valueOf(idx[0]) + "," + String.valueOf(idx[1]);
                        }
                        Val = observationTriple2[i].remove(key);
                        if (Val != null) {
                            observationTriple2[i].put(key, Val.doubleValue() + pair.getValue().doubleValue());
                        } else {
                            observationTriple2[i].put(key, pair.getValue().doubleValue());
                        }
                    }
                }
                observationTriplePrime = new HashMap<String, Double>();
                it = observationTriple.entrySet().iterator();
                sum = 0;
                while (it.hasNext()) {
                    Map.Entry<String, Double> pair = (Map.Entry<String, Double>) it.next();
                    ...
                    for (int i = 0; i < 3; i++) {
                        ...
                        Double Val2 = observationTriple2[i].get(key);
                        Double Val1 = observationTriple1[i].get(String.valueOf(idx[i]));
                        if ((Val1 != null) & (Val2 != null)) {
                            moment2 += Val2.doubleValue() * Val1.doubleValue();
                        }
                        if (Val1 != null) {
                            moment3 *= Val1.doubleValue();
                        }
                    }
                    moment2 /= 3.0;
                    double moment = lambda * moment1 + (1 - lambda) * (lambda * moment2 + (1 - lambda) * moment3);
                    sum += moment;
                    observationTriplePrime.put(String.valueOf(idx[0]) + "," + String.valueOf(idx[1]) + "," + String.valueOf(idx[2]), moment);
                }
                it = observationTriplePrime.entrySet().iterator();
                while (it.hasNext()) {
                    Map.Entry<String, Double> pair = (Map.Entry<String, Double>) it.next();
                    observationTriplePrime.put(pair.getKey(), pair.getValue().doubleValue() / sum);
                }
                observationTriple.clear();
                for (int i = 0; i < 3; i++) {
                    observationTriple1[i].clear();
                    observationTriple2[i].clear();
                }
            } else {
                observationPairPrime21 = observationPair21;
                observationPairPrime31 = observationPair31;
                observationTriplePrime = observationTriple;
            }
            HashMap<String,Double>[] observationTriplePer = new HashMap[nobserved]; // P_{3,x,1}
            for (int ni=0; ni<nobserved; ni++) {
                observationTriplePer[ni] = new HashMap<String,Double>();
            }
            it = observationTriplePrime.entrySet().iterator();
            while (it.hasNext()) {
                Map.Entry<String,Double> pair = (Map.Entry<String,Double>)it.next();
                ...
                int k1 = Integer.valueOf(st.nextToken()).intValue();
                int k2 = Integer.valueOf(st.nextToken()).intValue();
                int k3 = Integer.valueOf(st.nextToken()).intValue();
                observationTriplePer[k2].put(String.valueOf(k3)+","+String.valueOf(k1),pair.getValue().doubleValue());
            }

            Matrix observationInitialMatrix = new Matrix(nobserved,1,0);
            for (int nfile=0; nfile<chromfiles.length; nfile++) {
                int nobs = traindataObservedIndex[nfile][0];
                observationInitialMatrix.set(transObs[nobs],0,observationInitialMatrix.get(transObs[nobs],0)+1/(float)chromfiles.length);
            }
            double su = 1;
            for (int ni=0; ni<nobserved; ni++) {
                if (observationInitialMatrix.get(ni,0)==0) {
                    observationInitialMatrix.set(ni,0,1e-20);
                    su += 1e-20;
                }
            }
            for (int ni=0; ni<nobserved; ni++) {
                observationInitialMatrix.set(ni,0,observationInitialMatrix.get(ni,0)/su);
            }
//            System.out.println("read data");
            ...
            if (szoutfileID.equals(""))
            {
                szfile0 = szoutputdir+"/sample_moments_"+numdatasets+".txt";
            }
            else
            {
                szfile0 = szoutputdir+"/sample_moments_"+szoutfileID+"_"+numdatasets+".txt";
            }
            pw0 = new PrintWriter(szfile0);
            pw0.println("number_segments");
            pw0.println(nObservation);
            pw0.println("Marks");
            for (int nd=0; nd<numdatasets; nd++) {
                pw0.println(datasets[nd]);
            }
            pw0.println("single_vector");
            for (int ni=0; ni<nobserved; ni++) {
                pw0.println(transObs02[ni]+"\t"+observationSingleMatrix.get(ni,0));
            }
            pw0.println("initial_single_vector");
            for (int ni=0; ni<nobserved; ni++) {
                pw0.println(transObs02[ni]+"\t"+observationInitialMatrix.get(ni,0));
            }
            pw0.println("pair21_matrix");
            it = observationPairPrime21.entrySet().iterator();
            while (it.hasNext()) {
                Map.Entry<String, Double> pair = (Map.Entry<String, Double>) it.next();
                StringTokenizer st = new StringTokenizer(pair.getKey(), ",");
                int k1 = Integer.valueOf(st.nextToken()).intValue();
                int k2 = Integer.valueOf(st.nextToken()).intValue();
                pw0.println(transObs02[k2]+"\t"+transObs02[k1]+"\t"+pair.getValue().doubleValue());
            }
            pw0.println("pair31_matrix");
            it = observationPairPrime31.entrySet().iterator();
            while (it.hasNext()) {
                Map.Entry<String, Double> pair = (Map.Entry<String, Double>) it.next();
                StringTokenizer st = new StringTokenizer(pair.getKey(), ",");
                int k1 = Integer.valueOf(st.nextToken()).intValue();
                int k3 = Integer.valueOf(st.nextToken()).intValue();
                pw0.println(transObs02[k3]+"\t"+transObs02[k1]+"\t"+pair.getValue().doubleValue());
            }
            pw0.println("triple31_matrix");
            for (int k2=0; k2<nobserved; k2++) {
                it = observationTriplePer[k2].entrySet().iterator();
                while (it.hasNext()) {
                    Map.Entry<String, Double> pair = (Map.Entry<String, Double>) it.next();
                    StringTokenizer st = new StringTokenizer(pair.getKey(), ",");
                    int k1 = Integer.valueOf(st.nextToken()).intValue();
                    int k3 = Integer.valueOf(st.nextToken()).intValue();
                    pw0.println(transObs02[k2]+"\t"+transObs02[k3]+"\t"+transObs02[k1]+"\t"+pair.getValue().doubleValue());
                }
            }
            pw0.close();
            System.out.println("wrote sample moments");
						if (computesamplemomentonly) {
							return;
						}

            // Spectral learning
            Matrix observationPairMatrix31 = HashMap2Matrix(observationPairPrime31,nobserved);

            SingularValueDecomposition SVD = observationPairMatrix31.svd();
            Matrix U = SVD.getU().getMatrix(0,nobserved-1,0,numstates-1);


//        System.out.println("U rank: "+U.rank());
            Matrix V = SVD.getV().getMatrix(0,nobserved-1,0,numstates-1);
            double[] singular = SVD.getSingularValues();
//            Matrix diagS = diagonalize(singular,numstates);

            Matrix U_T = U.transpose();
//            Matrix Pair_inv3 = V.times(diagS.inverse());    // (U^T P_{3,1})^+
            Matrix Pair_inv3 = new Matrix(nobserved,numstates,0);
            for (int j=0; j<numstates; j++) {
            	for (int i=0; i<nobserved; i++) {
								Pair_inv3.set(i,j,V.get(i,j)/singular[j]);
							}
						}

            Matrix eigenvectors = new Matrix(numstates,numstates);
            if (ParameterSpectralLearning==SPECTRAL_DETERMINISTIC) {
                int[] mIdx = new int[numstates];
                for (int i=0; i<numstates; i++) {
                    ...
                    for (int j=0; j<nobserved; j++) {
                        ...
                        for (int i2=0; i2<i; i2++) {
                            if (mIdx[i2]==j)    {   exist = true;   break;  }
                        }
                        if (exist) {    continue;   }
                        double val = Math.pow(U.get(j,i),2);  // anchor observation
//          double val = observationSingleMatrix.get(j,0);  // most common observation
                        if (val>mV) { mV = val;   mI = j;  }
                    }
                    mIdx[i] = mI;
                }

                for (int i=0; i<numstates; i++) {
					Matrix Cx = MatrixProduct2(U_T,observationTriplePer[mIdx[i]],Pair_inv3);
                    EigenvalueDecomposition EIGx = Cx.eig();
                    double[] eigenval = EIGx.getRealEigenvalues();
                    Matrix eigenvec = EIGx.getV();
                    double mV = 0;
                    int mI = 0;
                    for (int j=0; j<numstates; j++) {
                        if (eigenval[j]>mV) {
                    ...
                    for (int j=0; j<numstates; j++) {
                        eigenvectors.set(j,i,eigenvec.get(j,mI));
                    }
                }
            } else if (ParameterSpectralLearning==SPECTRAL_RANDOM) {
                Random Rand = new Random(System.currentTimeMillis());
                double[] G = new double[nobserved];
                double norm1 = 0;
                for (int nobs = 0; nobs < nobserved; nobs++) {
                    G[nobs] = Rand.nextGaussian();
                    norm1 += Math.pow(G[nobs],2);
                }
                for (int nobs = 0; nobs < nobserved; nobs++) {
                    G[nobs] /= Math.sqrt(norm1);
                }
                Matrix C = new Matrix(numstates,numstates,0);
                for (int nobs = 0; nobs < nobserved; nobs++) {
                    Matrix Triple = MatrixProduct(U_T,observationTriplePer[nobs]);
                    Matrix Cx = Triple.times(Pair_inv3);
                    C = C.plus(Cx.times(G[nobs]));
                }
                EigenvalueDecomposition EIG = C.eig();
                eigenvectors = EIG.getV();
            }

            Matrix eigenvectors_inv = eigenvectors.inverse();

            // Infer Emission Matrix
            Matrix emissionMatrix = new Matrix(nobserved,numstates);
            Matrix C2 = new Matrix(numstates,numstates,0);
            for (int nobs = 0; nobs < nobserved; nobs++) {
//                Matrix Triple = MatrixProduct(U_T,observationTriplePer[nobs]);
//                Matrix Cx = Triple.times(Pair_inv3);
								Matrix Cx = MatrixProduct2(U_T,observationTriplePer[nobs],Pair_inv3);
                Matrix Ox = eigenvectors_inv.times(Cx).times(eigenvectors);
                for (int ns=0; ns<numstates; ns++) {
                    emissionMatrix.set(nobs,ns,Math.abs(Ox.get(ns,ns)));
                }
            }
            for (int ns=0; ns<numstates; ns++) {
                double s = 0;
                for (int nobs = 0; nobs < nobserved; nobs++) {
                    s += emissionMatrix.get(nobs,ns);
                }
                for (int nobs = 0; nobs < nobserved; nobs++) {
                    emissionMatrix.set(nobs,ns,emissionMatrix.get(nobs,ns)/s);
                }
            }
            ...
            if (szoutfileID.equals(""))
            {
                szfile = szoutputdir+"/emissions_comb_"+numstates+".txt";
                pw = new PrintWriter(szfile);
            }
            else
            {
                szfile = szoutputdir+"/emissions_comb_"+numstates+"_"+szoutfileID+".txt";
                pw = new PrintWriter(szfile);
            }
            pw.print("Observation\\State");
            for (int ns=0; ns<numstates; ns++) {
                pw.print("\t"+(ns+1));
            }
            pw.println();
            for (int nj=0; nj<nobserved0; nj++) {
                pw.print(nj);
                for (int ns = 0; ns < numstates; ns++) {
                    if (transObs0[nj] == -1) {
                        pw.print("\t" + nf.format(0));
                        continue;
                    }
                    if (transObs[transObs0[nj]] == -1) {
                        pw.print("\t" + nf.format(0));
                        continue;
                    }
                    pw.print("\t" + nf.format(emissionMatrix.get(transObs[transObs0[nj]], ns)));
                }
                pw.println();
            }
            pw.close();
            ...
            Matrix U_T_O = U_T.times(emissionMatrix);
            Matrix emissionMatrix_inv = emissionMatrix.inverse();

            double[][] emissionPer = new double[numstates][numdatasets];
            for (int ns=0; ns<numstates; ns++) {
                emissionPer[ns] = new double[numdatasets];
                for (int ni=0; ni<numdatasets; ni++) {
                    emissionPer[ns][ni] = 0.0;
                }
                for (int nj=0; nj<nobserved; nj++) {
                    for (int ni=0; ni<numdatasets; ni++) {
                        if (traindataNotMissing[transObs2[nj]][ni]) {
                            if (traindataObservedValues[transObs2[nj]][ni]) {
                                emissionPer[ns][ni] += emissionMatrix.get(nj, ns);
                            }
                        }
                    }
                }
            ...
            if (isComb) {
                ...
                for (int nj = 0; nj < traindataObservedValues.length; nj++) {
                    ...
                    for (int ns = 0; ns < numstates; ns++) {
                        if (transObs[nj] != -1) {
                            emissionproductsSpectral[nj][ns] = Math.max(emissionMatrix.get(transObs[nj], ns),1e-20);	
                        } else {
                            emissionproductsSpectral[nj][ns] = 1e-20;
                        }
                    }
            ...
            // Infer initial state distribution vector.

            Matrix Pi = emissionMatrix_inv.times(observationInitialMatrix); // Pi = (O^+) P^{init}_1
            double s = 0;
            for (int ns=0; ns<numstates; ns++) {
                Pi.set(ns,0,Math.abs(Pi.get(ns,0)));
                s += Pi.get(ns,0);
            }
            for (int ns=0; ns<numstates; ns++) {
                Pi.set(ns,0,Pi.get(ns,0)/s);
            }

            // Inter transition Matrix.
            Matrix T1 = MatrixProduct(emissionMatrix_inv,observationPairPrime31);
            Matrix T2 = MatrixProduct(emissionMatrix_inv,observationPairPrime21).transpose().inverse().transpose();
            Matrix transitionMatrix = T1.times(T2); // T = (O^+ P_{3,1}) (O^+ P_{2,1})^+

/*
        Matrix avgPi = emissionMatrix_inv.times(observationSingleMatrix); // Pi = (O^+)P^1
        s = 0;
        for (int ns=0; ns<numstates; ns++) {
            avgPi.set(ns,0,Math.abs(avgPi.get(ns,0)));
            s += avgPi.get(ns,0);
        }
        for (int ns=0; ns<numstates; ns++) {
            avgPi.set(ns,0,avgPi.get(ns,0)/s);
        }
        Matrix diagPi = new Matrix(numstates,numstates,0);
        for (int ns=0; ns<numstates; ns++) {
            diagPi.set(ns,ns,avgPi.get(ns,0));
        }
        Matrix transitionMatrix = MatrixProduct(emissionMatrix_inv,observationPairPrime21).times(emissionMatrix_inv.transpose()).times(diagPi.inverse()); // T = O^+P_{2,1}O^Tdiag(pi)^{-1}
*/
            for (int ns=0; ns<numstates; ns++) {
                for (int ni=0; ni<numstates; ni++) {
                    transitionMatrix.set(ns,ni,Math.abs(transitionMatrix.get(ns,ni)));
                }
            }
            for (int ns=0; ns<numstates; ns++) {
                s = 0;
                for (int ni=0; ni<numstates; ni++) {
                    s += transitionMatrix.get(ni,ns);
                }
                for (int ni=0; ni<numstates; ni++) {
                    transitionMatrix.set(ni,ns,transitionMatrix.get(ni,ns)/s);
//										System.out.print(transitionMatrix.get(ni,ns)+" ");
                }
//								System.out.println();
            }
//        Pi = avgPi;
//        System.out.println("T rank: "+transitionMatrix.rank());
            // Write parameters
            emissionprobs = new double[numstates][numdatasets][numbuckets];
            for (int ns=0; ns<numstates; ns++) {
                emissionprobs[ns] = new double[numdatasets][numbuckets];
                for (int ni=0; ni<numdatasets; ni++) {
                    emissionprobs[ns][ni] = new double[numbuckets];
                    emissionprobs[ns][ni][0] = Math.min(1-1e-20,Math.max(1e-20,(1-emissionPer[ns][ni])));
                    emissionprobs[ns][ni][1] = Math.min(1-1e-20,Math.max(1e-20,emissionPer[ns][ni]));
                }
            }
            transitionprobs = transitionMatrix.transpose().getArray();
            probinit = new double[numstates];
            for (int ns=0; ns<numstates; ns++) {
                probinit[ns] = Pi.get(ns,0);
            }
            //set to true if a transition has been eliminated
            elim = new boolean[numstates][numstates];
            //initialize index of the next non-zero transition
    ...
    // Added by JS
    public Matrix HashMap2Matrix(HashMap<String, Double> Map, int size) {
        ...
        while (it.hasNext()) {
            ...
            int k1 = Integer.valueOf(st.nextToken()).intValue();
            int k2 = Integer.valueOf(st.nextToken()).intValue();
            M.set(k2,k1,M.get(k2,k1)+pair.getValue().doubleValue());
            n++;
        }
        return M;
    }
    ...
    // Added by JS
    public Matrix diagonalize(double[] array, int size) {
        int size2 = Math.min(size,array.length);
        Matrix M = new Matrix(size2,size2,0);
    ...
    // Added by JS
    public Matrix MatrixProduct(Matrix A, HashMap<String,Double> Map) {	// A * matrix(Map)
        int nrow = A.getRowDimension();
        int ncol = A.getColumnDimension();
        Matrix M = new Matrix(nrow,ncol,0);
        Iterator it = Map.entrySet().iterator();
        while (it.hasNext()) {
            Map.Entry<String,Double> pair = (Map.Entry<String,Double>)it.next();
            StringTokenizer st = new StringTokenizer(pair.getKey(),",");
            ...
            for (int i=0; i<nrow; i++) {
                M.set(i,k2,M.get(i,k2)+A.get(i,k1)*pair.getValue().doubleValue());
            }
        }
        return M;
    }
    ...
    // Added by JS
    public Matrix MatrixProduct2(Matrix A, HashMap<String,Double> Map, Matrix B) { // A * matrix(Map) * B
        int nrow = A.getRowDimension();
        int ncol = B.getColumnDimension();
        Matrix M = new Matrix(nrow,ncol,0);
        Iterator it = Map.entrySet().iterator();
        while (it.hasNext()) {
            Map.Entry<String,Double> pair = (Map.Entry<String,Double>)it.next();
            ...
            for (int i=0; i<nrow; i++) {
                ...
                M.set(i,j,M.get(i,j)+A.get(i,k1)*pair.getValue().doubleValue()*B.get(k2,j));
							}
            }
        }
        return M;
	...
	Random theRandom2 = new Random(999);	// Added by JS
	for (int ni = 0; ni < chromfiles.length; ni++)
	{
	   tempchromfiles[ni] = chromfiles[ni];
//	   if (theRandom == null)
	   if (theRandom2 == null)	// Added by JS
	   {
...
//	      recA[ni] = new RecIntDouble(ni,theRandom.nextDouble());
	      recA[ni] = new RecIntDouble(ni,theRandom2.nextDouble());	// Added by JS
	   }
	}
...
//	if (theRandom != null)
	...
	{
	    System.out.println("This is Version 1.10 of Spectacle (c) Copyright 2008-2012 Massachusetts Institute of Technology");
	}
        else if (szcommand.equalsIgnoreCase("BinarizeBed"))
	{
	    ...
	    int npseudocount = 1;
	    int nbinsize = Spectacle.DEFAULT_BINSIZEBASEPAIRS;
	...
	else if (szcommand.equalsIgnoreCase("CompareModels"))
	{
	    ...
	    int nr=Spectacle.DEFAULTCOLOR_R;
	    int ng=Spectacle.DEFAULTCOLOR_G;
	    int nb=Spectacle.DEFAULTCOLOR_B;
	...
	else if (szcommand.equalsIgnoreCase("EvalSubset"))
	{
	    ...
	    if (bok&&(nargindex==args.length-5))
	    {
	       ...
	       {
		   Spectacle theHMM = new Spectacle(szinputdir, szsegmentdir,szinputfilelist,szconfusionfileprefix, 
	...
	else if (szcommand.equalsIgnoreCase("MakeSegmentation"))
	{
	    ...
	    int nbinsize = Spectacle.DEFAULT_BINSIZEBASEPAIRS;
	    String szoutfileID = "";
      boolean isComb = false;	// Added by JS
            ...
            try
	    {
		  ...
		  else if (args[nargindex].equals("-printstatesbyline"))
		  {
			...
			// Added by JS // 
			else if (args[nargindex].equals("-comb"))
			{
				isComb = true;
			}
			// Added by JS // 
	    ...
	    if (bok&&(nargindex==args.length-3))
	    {
		   ...
		   Spectacle theHMM = new Spectacle(szinputdir, szinputfilelist,szchromlengthfile, szoutputdir, szmodelfile, szoutfileID, nbinsize, bprintposterior,
//                                              bprintsegments,bprintstatebyline);	
                                              bprintsegments,bprintstatebyline,isComb);		// Added by JS

//	          theHMM.makeSegmentation();
               theHMM.makeSegmentation(isComb);   // Added by JS
	       }
	       else
	       {
	...
	{	    
	    ...
	    String szlabelmapping = null;
	    int noffsetleft = Spectacle.DEFAULT_OVERLAPENRICHMENT_NOFFSETLEFT;   //int Spectacle.DEFAULT_OVERLAPENRICHMENT_NOFFSETLEFT = 0;
	    int noffsetright = Spectacle.DEFAULT_OVERLAPENRICHMENT_NOFFSETRIGHT;  //int Spectacle.DEFAULT_OVERLAPENRICHMENT_NOFFSETRIGHT = 1;
	    int nbinsize = Spectacle.DEFAULT_BINSIZEBASEPAIRS; //
	    boolean bcenter = Spectacle.DEFAULT_OVERLAPENRICHMENT_BCENTER; //boolean Spectacle.DEFAULT_OVERLAPENRICHMENT_BCENTER = false;
	    boolean bcountmulti = Spectacle.DEFAULT_OVERLAPENRICHMENT_BCOUNTMULTI;  //boolean Spectacle.DEFAULT_OVERLAPENRICHMENT_BCOUNTMULTI = false;
	    boolean busesignal = Spectacle.DEFAULT_OVERLAPENRICHMENT_BUSESIGNAL;  //boolean Spectacle.DEFAULT_OVERLAPENRICHMENT_BUSESIGNAL = false;
	    String szcolfields = null; 
	    boolean bbaseres = Spectacle.DEFAULT_OVERLAPENRICHMENT_BBASERES; //boolean Spectacle.DEFAULT_OVERLAPENRICHMENT_BBASERES = true;
	    String szoutfile;
	    boolean buniformheat = Spectacle.DEFAULT_OVERLAPENRICHMENT_BUNIFORMHEAT; //boolean Spectacle.DEFAULT_OVERLAPENRICHMENT_BUNIFORMHEAT = false;
	...
	else if ((szcommand.equalsIgnoreCase("NeighborhoodEnrichment"))||(szcommand.equalsIgnoreCase("NeighborhoodSignal")))
	{
	    ...
	    int numleft = Spectacle.DEFAULT_NEIGHBORHOOD_NUMLEFT; 
	    int numright = Spectacle.DEFAULT_NEIGHBORHOOD_NUMRIGHT; 
	    int nspacing = Spectacle.DEFAULT_BINSIZEBASEPAIRS;
	    String szlabelmapping = null;
	    boolean busestrand =Spectacle.DEFAULT_NEIGHBORHOOD_BUSESTRAND; 
	    boolean busesignal = Spectacle.DEFAULT_NEIGHBORHOOD_BUSESIGNAL; 
	    String szcolfields = null;
	    int noffsetanchor = Spectacle.DEFAULT_NEIGHBORHOOD_NOFFSETANCHOR; 
	    String szoutfile; 
	    String sztitle = "Fold Enrichments";
	    String szcell = ""; 
        ...
        {
	    String path = Spectacle.class.getProtectionDomain().getCodeSource().getLocation().getPath();
...
//	    int nseed = 999;
			int nseed = (int)(System.currentTimeMillis()%1000000);	// Added by JS
//	    int nmaxiterations= 200;//number of passes through all the data
	    int nmaxiterations= 0;//number of passes through all the data	// Added by JS
			boolean nmaxiterations_default = true;  // Added by JS
	    int nzerotransitionpower =8; //Sets the transition probability to 0 if below this constant value
	    String szInitFile =null;
	    int ninitmethod = Spectacle.INITMETHOD_INFORMATION;
	    ...
	    int nbinsize = Spectacle.DEFAULT_BINSIZEBASEPAIRS;
	    String szoutfileID = "";
	    int nstateorder =Spectacle.STATEORDER_EMISSION;
	    ...
	    boolean bnormalEM = false;
			// Added by JS // 
			int ParameterSpectralLearning = SPECTRAL_DETERMINISTIC; 
			double lambda = 1.0; 
      boolean isComb = false;
            ...
            try
	    {
		  ...
		  else if (args[nargindex].equals("-init"))
		  {
		     ...
		     {
			 ninitmethod = Spectacle.INITMETHOD_INFORMATION;
		     }
		     else if ((sztoken.equals("random"))||(sztoken.equals("1")))
		     {
			 ninitmethod = Spectacle.INITMETHOD_RANDOM;
		     } 
             else if ((sztoken.equals("load"))||(sztoken.equals("2")))
		     {
			 ninitmethod = Spectacle.INITMETHOD_LOAD;
		     }
             // Added by JS //
             else if ((sztoken.equals("load_comb"))||(sztoken.equals("3")))
             {
                 ninitmethod = Spectacle.INITMETHOD_LOAD_COMB;
                 isComb = true;
             }
             // Added by JS //
		     else
		  ...
		  else if (args[nargindex].equals("-stateordering"))
		  {
		     ...
		     {
			 nstateorder = Spectacle.STATEORDER_EMISSION;
		     } 
                     else if (sztoken.equals("transition"))
		     {
			 nstateorder = Spectacle.STATEORDER_TRANSITION;
		     }
		     else
		     {
			 bok = false;
		  ...
		  else if (args[nargindex].equals("-r"))
		  {
		     nmaxiterations = Integer.parseInt(args[++nargindex]);
				 nmaxiterations_default = false;  // Added by JS
		  }	
		  else if (args[nargindex].equals("-s"))
		  {
		  ...
		  // Added by JS //
		  else if (args[nargindex].equals("-chromhmm"))
		  {
		 	 ParameterSpectralLearning = EXPECTATION_MAXIMIZATION;
		  }
		  else if (args[nargindex].equals("-spectralrandom"))
		  {
			 ParameterSpectralLearning = SPECTRAL_RANDOM;
		  }
		  else if (args[nargindex].equals("-lambda"))
		  {
		 	 lambda = Double.parseDouble(args[++nargindex]);
		  }
          else if (args[nargindex].equals("-comb"))
          {
              isComb = true;
          }
			else if (args[nargindex].equals("-computesamplemomentonly"))
			{
                computesamplemomentonly = true;
			}
		  // Added by JS //
		  else
		  {
		...
		bok = false;
			...
			// Added by JS //
			if ((ParameterSpectralLearning==EXPECTATION_MAXIMIZATION)&(nmaxiterations_default==true)) {
				nmaxiterations = 200;
			} else if ((isComb==true)&(nmaxiterations_default==true)) {
                nmaxiterations = 0;
            }
			// Added by JS //
	    ...
	    if (bok&&(nargindex==args.length-4))
	    {
		  ...
		  if ((szoutfileID.equals(""))&&(ninitmethod == Spectacle.INITMETHOD_RANDOM))
		  {
		      szoutfileID = ""+nseed;
		  }
		  ...
		  if (szchromlengthfile == null)
		  {
			  ...
			  szchromlengthfile = szprefixpath+"/"+CHROMSIZESDIR+"/"+szassembly+".txt";
 	          ...
 	          Spectacle theHMM = new Spectacle(szinputdir, szoutputdir, szinputfilelist,szchromlengthfile,numstates, nseed,ninitmethod,
...
//						 nbinsize,szoutfileID,nstateorder,bordercols,nzerotransitionpower,theColor,bnormalEM, nmaxprocessors,ParameterSpectralLearning,lambda,isComb);	// Added by JS
						 nbinsize,szoutfileID,nstateorder,bordercols,nzerotransitionpower,theColor,bnormalEM, nmaxprocessors,ParameterSpectralLearning,lambda,isComb,computesamplemomentonly);	// Added by JS
	          theHMM.buildModel();

//		  String szwebpage = szoutputdir+"/webpage_"+numstates+".html";

	          if (computesamplemomentonly==false) {	// Added by JS
	  			// Added by JS //
	        	  String szoutfileID2 = "";
      if (szoutfileID!="") {
        szoutfileID2 = "_"+szoutfileID;
      }
      String szwebpage = szoutputdir+"/webpage_"+numstates+szoutfileID2+".html";
			// Added by JS //
		  PrintWriter pwweb = new PrintWriter(new FileWriter(szwebpage));


		  pwweb.println("<center><h1>Spectacle Report</h1></center>");
		  ...
		  pwweb.print("Full Spectacle command: ");
...
*/
			// Added by JS //
		  pwweb.println("<img src=\"emissions_"+numstates+szoutfileID2+".png\"><br>");
		  pwweb.println("<li><a href=\"emissions_"+numstates+szoutfileID2+".svg\">Emission Parameter SVG File</a><br>");
		  pwweb.println("<li><a href=\"emissions_"+numstates+szoutfileID2+".txt\">Emission Parameter Tab-Delimited Text File</a><br>");
		  pwweb.println("<img src=\"transitions_"+numstates+szoutfileID2+".png\"><br>");
		  pwweb.println("<li><a href=\"transitions_"+numstates+szoutfileID2+".svg\">Transition Parameter SVG File</a><br>");
		  pwweb.println("<li><a href=\"transitions_"+numstates+szoutfileID2+".txt\">Transition Parameter Tab-Delimited Text File</a><br><br>");
		  pwweb.println("<li><a href=\"model_"+numstates+szoutfileID2+".txt\">All Model Parameters Tab-Delimited Text File</a> <br>");
			// Added by JS //
...
//	             theHMM.makeSegmentation();
			...
			{
			   String szsegmentfile = prefixes[nfile]+Spectacle.SZSEGMENTEXTENSION;
			...
			if (bprintbrowser)
			{
			    ...
			    for (int nprefixindex = 0; nprefixindex < prefixes.length; nprefixindex++)
			    {
			      ...
			      String szsegmentfile = szoutputdir+"/"+szprefix+Spectacle.SZSEGMENTEXTENSION;
			      ...
			      pwweb.println("<li><a href="+szprefix+Spectacle.SZBROWSERDENSEEXTENSION+".bed>"+szprefix+" Browser Custom Track Dense File</a> <br>");
			      pwweb.println("<li><a href="+szprefix+Spectacle.SZBROWSEREXPANDEDEXTENSION+".bed>"+szprefix+" Browser Custom Track Expanded File</a><br>");
			    ...
			    for (int nprefixindex = 0; nprefixindex < prefixes.length; nprefixindex++)
			    {
		               ...
		               if (fcoordassembly.exists())
			       {
			          StateAnalysis.enrichmentMax(szsegmentfile,szprefixpath+"/"+COORDDIR+"/"+szassembly,null,//szinputcoordlist,
                                                          Spectacle.DEFAULT_OVERLAPENRICHMENT_NOFFSETLEFT,
                                                          Spectacle.DEFAULT_OVERLAPENRICHMENT_NOFFSETRIGHT,nbinsize,  
							  Spectacle.DEFAULT_OVERLAPENRICHMENT_BCENTER, !Spectacle.DEFAULT_OVERLAPENRICHMENT_BCOUNTMULTI, 
                                                          Spectacle.DEFAULT_OVERLAPENRICHMENT_BUSESIGNAL,null,//szcolfields,
                                                          Spectacle.DEFAULT_OVERLAPENRICHMENT_BBASERES, szoutputdir+"/"+szprefix+Spectacle.SZOVERLAPEXTENSION,
							      !Spectacle.DEFAULT_OVERLAPENRICHMENT_BUNIFORMHEAT,theColor,"Fold Enrichment "+szprefix,null);
				  String szoverlapoutfile = szprefix+Spectacle.SZOVERLAPEXTENSION+".txt";
				  pwweb.println("<img src=\""+szprefix+Spectacle.SZOVERLAPEXTENSION+".png\"> <br>");
				  pwweb.println("<li><a href=\""+szprefix+Spectacle.SZOVERLAPEXTENSION+".svg"+"\">"+szprefix+" Overlap Enrichment SVG File"+"</a><br>");
				     ...
				     else
				     {
				        szanchorname = dir[nfile].substring(0,nperiodindex);
			             ...
			             StateAnalysis.neighborhoodMax(szsegmentfile,szprefixpath+"/"+
                                                                 ANCHORFILEDIR+"/"+szassembly+"/"+dir[nfile],nbinsize,Spectacle.DEFAULT_NEIGHBORHOOD_NUMLEFT,
							     Spectacle.DEFAULT_NEIGHBORHOOD_NUMRIGHT, nbinsize,//nspacing
                                        Spectacle.DEFAULT_NEIGHBORHOOD_BUSESTRAND,Spectacle.DEFAULT_NEIGHBORHOOD_BUSESIGNAL,null,//szcolfields,
                                        Spectacle.DEFAULT_NEIGHBORHOOD_NOFFSETANCHOR,szoutputdir+"/"+szprefix+"_"+szanchorname+"_neighborhood",
								   theColor,"Fold Enrichment "+szprefix+" "+szanchorname,null);
				     String szneighborhoodoutfileprefix = szprefix+"_"+szanchorname+Spectacle.SZNEIGHBORHOODEXTENSION;
...
//                                   "[-t loadsmoothtransition][-x maxseconds][-z zerotransitionpower] inputdir outputdir numstates assembly");
                                   "[-t loadsmoothtransition][-x maxseconds][-z zerotransitionpower][-chromhmm][-lambda lambdavalue][-spectralrandom] inputdir outputdir numstates assembly");	// Added by JS
	    }
	} 
	    ...
	    int nstateorder = Spectacle.STATEORDER_FIXED;
	    boolean bnoordercols = false;
	    boolean bnoprintsegment = false;
	    int nargindex = 1;
            ...
            try
	    {
		  ...
		  else if (args[nargindex].equals("-o"))
		  {
		      ...
		      nstateorder = Spectacle.STATEORDER_USER;
		  }
		  else if (args[nargindex].equals("-stateordering"))
		  {
                     String sztoken = args[++nargindex];
	    ...
	    if (bok&&(nargindex==args.length-2))
	    {
	       ...
	       if (bok)
	       {
	          boolean bprintsegments = !bnoprintsegment; 
	          boolean bordercols = !bnoordercols;
		  Spectacle theHMM = new Spectacle(szInitFile, szoutputdir, szstateorderingfile, szcolumnorderingfile, szoutfileID,nstateorder,bordercols,theColor,szlabelmapping);
		  theHMM.reorderModel();
	       }
	    }
	    else
